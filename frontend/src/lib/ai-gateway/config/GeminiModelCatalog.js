...
export function validateGeminiModelIdentifier(value, { allowGemmaShadow = true } = {}) {
...
export function validateGeminiProductionRoute(modelIds = GEMINI_PRODUCTION_MODEL_IDS) {
  ...
  const records = ids.map((model) => validateGeminiModelIdentifier(model, { allowGemmaShadow: false }));
