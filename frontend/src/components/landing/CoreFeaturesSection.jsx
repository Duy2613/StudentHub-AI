      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16 space-y-3">
          <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-teal-500/10 border border-teal-500/30 text-teal-300 text-xs font-bold uppercase tracking-wider">
            <Layers className="w-3.5 h-3.5 text-teal-400" /> 3 Trụ Cột Nền Tảng
          </div>
          <h2 className="text-2xl sm:text-4xl font-black text-white tracking-tight">
            Giải Pháp Toàn Diện Phòng Chống Lừa Đảo
          </h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {features.map((feat) => {
            ...
            return (
              <div
                key={feat.id}
                className="p-8 rounded-3xl bg-white/[0.03] hover:bg-white/[0.06] border border-white/10 hover:border-teal-500/40 backdrop-blur-2xl transition-all duration-300 flex flex-col justify-between shadow-glass-deep group hover:-translate-y-1"
              >
                <div className="space-y-5">
                  <div className="flex items-center justify-between">
                    <div className="p-3.5 rounded-2xl bg-teal-500/15 text-teal-300 border border-teal-500/30 group-hover:scale-110 transition-transform">
                      <Icon className="w-6 h-6" />
                    </div>
                    <span className={`text-[10px] font-bold uppercase px-3 py-1 rounded-full border ${feat.tagColor}`}>
                      {feat.badge}
                    </span>
                  </div>
                  ...
                  <ul className="space-y-2.5 pt-2 border-t border-white/5 text-xs text-gray-300">
