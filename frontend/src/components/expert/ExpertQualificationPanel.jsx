...
export default function ExpertQualificationPanel() {
  ...
  return <section className="qualification-panel intelligence-panel" aria-labelledby="expert-qualification-title">
    ...
    {!loading && !authRequired && trackRecord && <div className="qualification-status-row" aria-label="Community contribution track record" role="status" aria-live="polite">
      ...
      <div><span className="data-label">Contribution track record · {trackRecord.policyVersion}</span><strong>{trackRecord.expertCandidate ? "Đủ điều kiện mở qualification" : "Chưa đủ điều kiện mở qualification"}</strong><small>{trackRecord.expertCandidate ? "Đã qua cổng cộng đồng — vẫn bắt buộc identity, quiz, practice và human activation." : "Tín hiệu cộng đồng chưa đủ để mở bước qualification; đây không phải phán quyết Trust."}</small></div>
      ...
      {practiceEligible && <div className="qualification-practice">
        ...
        {domains.filter((domainCode) => !submittedPracticeDomains.has(domainCode)).length > 0 && <form className="qualification-form" onSubmit={submitPractice}>
          ...
          <label><span>Bước tiếp theo</span><textarea rows={2} value={practiceNextAction} onChange={(event) => setPracticeNextAction(event.target.value)} maxLength={1200} placeholder="Người dùng nên xác minh hoặc làm gì tiếp?" required /></label>
           <label><span>Nguồn tham khảo cho reviewer</span><input value={practiceEvidence} onChange={(event) => setPracticeEvidence(event.target.value)} placeholder="Thêm các tham chiếu nguồn, phân tách bằng dấu phẩy" required /></label>
