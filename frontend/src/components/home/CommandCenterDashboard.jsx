...
export default function CommandCenterDashboard() {
  ...
  const actions = useMemo(() => {
    ...
    if (data?.nextBestAction) return [normalizePriority({ ...data.nextBestAction, urgency: "MEDIUM", href: data.nextBestAction.href || "/academic" }, 0)];
    return [];
  }, [data]);
    ...
    <div className="space-y-6">
      <section className="dashboard-hero surface-card overflow-hidden p-5 sm:p-7">
        <div className="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          ...
          <Link href={data?.nextBestAction?.href || "/academic"} className="primary-action shrink-0">Mở bước tiếp theo <ArrowRight size={16} /></Link>
        </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.5fr)_minmax(18rem,.8fr)]">
        <section className="surface-card p-5 sm:p-6" aria-labelledby="next-actions-title"><div className="flex items-end justify-between gap-4"><div><p className="eyebrow">Next best actions</p><h2 id="next-actions-title" className="section-title mt-2">Việc cần làm trước</h2></div><span className="text-xs text-app-muted">{actions.length} hàng có căn cứ</span></div>
          ...
          <div className="mt-5 flex flex-wrap gap-2"><Link href="/academic" className="text-link">Xem workflow học vụ <ArrowRight size={14} /></Link><Link href="/settings" className="text-link muted">Cài đặt quyền dữ liệu <ArrowRight size={14} /></Link></div>
        </section>

        <aside className="space-y-6"><section className="surface-card p-5"><div className="flex items-center gap-2"><CalendarClock size={16} className="text-teal-300" /><p className="eyebrow">Context hiện tại</p></div><p className="mt-3 text-sm leading-6 text-slate-300">{data?.nextBestAction?.description || "Chưa có bước tiếp theo được hệ thống xác định."}</p><Link href={data?.nextBestAction?.href || "/intelligence"} className="text-link mt-4">Mở căn cứ <ArrowRight size={14} /></Link></section><section className="surface-card p-5"><p className="eyebrow">Hỏi nhanh</p><h2 className="mt-2 text-lg font-bold text-white">Cần kiểm tra điều gì?</h2><div className="mt-4 flex gap-2"><input value={askPrompt} onChange={(event) => setAskPrompt(event.target.value)} placeholder="Ví dụ: điều kiện tốt nghiệp" className="field-input min-w-0 flex-1" aria-label="Câu hỏi cho AI" /><Link href={`/ai?q=${encodeURIComponent(askPrompt || "Điều kiện tốt nghiệp")}`} className="primary-icon" aria-label="Hỏi AI"><Sparkles size={16} /></Link></div><div className="mt-4 flex flex-wrap gap-2"><Link href="/ai?q=điều kiện tốt nghiệp" className="suggestion-chip">Điều kiện tốt nghiệp</Link><Link href="/trust" className="suggestion-chip">Kiểm tra bằng chứng</Link></div></section></aside>
      </div>

      <section className="surface-card grid gap-5 p-5 sm:grid-cols-[1fr_auto] sm:items-center sm:p-6"><div className="flex gap-3"><span className="stat-icon teal"><MessageCircle size={17} /></span><div><p className="eyebrow">Mạng lưới xác thực</p><h2 className="mt-2 text-lg font-bold text-white">Khi chưa chắc, hãy xem nguồn trước khi quyết định</h2><p className="mt-1 text-sm text-app-muted">{isExplicitDemo ? "Fixture minh họa Evidence Triangle — không đại diện cho cá nhân hay tổ chức thật." : "Trust, Expert, Community và Evidence được nối trong cùng một luồng giải thích."}</p></div></div><Link href={isExplicitDemo ? "/cases" : "/intelligence"} className="secondary-action justify-center"><UserRound size={14} /> {isExplicitDemo ? "Mở Demo Case Lab" : "Mở Intelligence Fabric"} <ArrowRight size={14} /></Link></section>
    </div>
  );
}
