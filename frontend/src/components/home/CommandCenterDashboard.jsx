...
export default function CommandCenterDashboard() {
  ...
  useEffect(() => {
    ...
    fetch("/api/v1/dashboard", { credentials: "include" })
      .then(async (response) => {
        ...
        if (payload?.data?.demo === true || payload?.data?.sourceState === "DEMO_FIXTURE") {
          throw new Error("Dashboard live data rejected an unexpected fixture response.");
        }
        return payload.data;
      })
      .then((payload) => {
  ...
  const sourceState = data?.sourceState || data?.explainability?.sourceState || "UNKNOWN";
  const sourceLabel = sourceState === "LIVE" ? "LIVE_SOURCE" : "SNAPSHOT / CẦN KẾT NỐI";
    ...
    return (
      <section className="surface-card p-6 sm:p-8" aria-labelledby="dashboard-auth-title">
        ...
        <div className="mt-5 flex flex-wrap gap-3">
          <Link href="/login?next=%2Fdashboard" className="primary-action">Đăng nhập <ArrowRight size={16} /></Link>
          <Link href="/trust" className="secondary-action">Mở Trust Engine <ArrowRight size={14} /></Link>
        </div>
      </section>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <section className="dashboard-hero surface-card overflow-hidden p-5 sm:p-7 relative group">
        {/* Ambient Dashboard Architectural Backdrop */}
        <div className="absolute inset-0 pointer-events-none overflow-hidden z-0">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img
            src={V3_MEDIA.dashboard.dashboard}
            alt=""
            className="w-full h-full object-cover opacity-20 filter contrast-125 saturate-120 scale-105"
          />
        ...
        <div className="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-2xl">
            <div className="flex flex-wrap items-center gap-2"><p className="eyebrow text-teal-300">VI · Quiet priority ledger</p><span className="metadata-chip" data-provider-status="clean">{sourceLabel}</span></div>
          ...
          <Link href={data?.nextBestAction?.href || "/trust"} className="primary-action shrink-0" data-cursor="MỞ BƯỚC TIẾP">Mở bước tiếp theo <ArrowRight size={16} /></Link>
        </div>
        <div className="mt-7 flex flex-wrap items-center gap-x-6 gap-y-2 border-t border-white/10 pt-4 text-xs text-app-muted relative z-10"><span className="flex items-center gap-2"><span className="status-dot" /> Nguồn: {sourceLabel}</span><span className="flex items-center gap-2"><ShieldCheck size={14} className="text-teal-300" /> Không có căn cứ thì không tạo cảnh báo</span></div>
      </section>
