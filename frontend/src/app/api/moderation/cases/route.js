...
export async function GET(request) {
  try {
    ...
    const cases = await repo.getModeratorCases({ status, limit });
...
export async function POST(request) {
  try {
    ...
    const { targetType, targetId, targetAuthorId, reasonCategory, details } = body;
    ...
    const isClientError = error.message?.includes("REQUIRED") || error.message?.includes("DISALLOWED");
