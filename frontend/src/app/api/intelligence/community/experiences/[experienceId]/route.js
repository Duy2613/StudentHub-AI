...
export const GET = SecurityFabric.wrapHandler({ action: "READ_COMMUNITY_EXPERIENCE", allowAnonymous: true, maxRequests: 90 }, async (_request, routeParams, _principal, secContext) => {
  ...
  try {
    const post = demoMode() ? CommunityStore.getPost(experienceId, { redactPrivate: true }) : await CommunityRepository.getContribution(experienceId);
    ...
    return Response.json({ success: true, experience: post, sourceState: demoMode() ? "DEMO_FIXTURE" : "COMMUNITY_SIGNAL", isAuthoritative: false, dataNotice: "Community contributions are signals attached to a case revision; Trust remains authoritative." });
  } catch (error) {
