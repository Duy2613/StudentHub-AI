 ...
 *
 * This is a new 150-case evaluation set generated after the RC3 retrieval
 * boundary was implemented. Runtime claims contain only query text and
 * resolver output. Gold domains are read only by the scorer after retrieval;
 * they never seed a provider request or candidate.
 */

const datasetPath = path.resolve("docs/evaluation/retrieval_fresh_holdout_v4_dataset.json");
...
function buildRuntimeClaim(item) {
  ...
  return {
    claimId: `holdout-v4-${item.caseId}`,
    text: item.query,
...
async function collectRuntimeCandidates() {
  ...
  const runtimeCandidates = await mapWithConcurrency(cases, async (item, index) => {
    ...
    const [staticResult, liveResult] = await Promise.all([
      EvidenceDiscoveryService.discoverEvidenceForClaims({
        claims: [claim],
        runId: `retrieval-v4-static-${index + 1}`,
        revision: 1,
        mode: "STATIC",
      }),
      EvidenceDiscoveryService.discoverEvidenceForClaims({
        claims: [claim],
        runId: `retrieval-v4-live-${index + 1}`,
        revision: 1,
        mode: "LIVE",
      }),
    ]);
  ...
  return { runtimeCandidates, officialDiscovery };
}
...
function evaluateMode(modeName, queryCases, runtimeCandidates) {
  const metricLists = {
    ...
    officialHitRate: [], officialTop3Rate: [], officialDiscoveryHitRate: [],
...
test("FRESH RETRIEVAL HOLDOUT V4 — RC3 PUBLIC/OFFICIAL DISCOVERY PATH (N=150 QUERIES)", async () => {
  assert.equal(cases.length >= 150, true, "RC3 retrieval holdout must contain at least 150 cases");

  console.log("\n============================================================");
  console.log("🔍 RUNNING FRESH RETRIEVAL HOLDOUT V4 — RC3 (N=150)");
  console.log("============================================================\n");

  const { runtimeCandidates, officialDiscovery } = await collectRuntimeCandidates();
  ...
  const status = Object.values(targetGates).every(Boolean)
    ...
    : "RETRIEVAL_HOLDOUT_V4_TARGETS_NOT_ESTABLISHED";

  console.log("🛡️ RC3 HYBRID POOL INVARIANTS:");
  ...
  const reportArtifact = {
    holdoutVersion: "RETRIEVAL_HOLDOUT_V4",
    sampleSize: cases.length,
    generatedDataset: "docs/evaluation/retrieval_fresh_holdout_v4_dataset.json",
    runtimeBoundary: "QUERY_TEXT_AND_RESOLVER_OUTPUT_ONLY; GOLD_DOMAINS_SCORING_ONLY",
    publicSourceIntegration: "OFFICIAL_DISCOVERY_ADAPTER_REAL_FETCH; DISCOVERY_ONLY",
    ...
    targetGates,
    status,
    evidenceClass: "NEW_RC3_RETRIEVAL_VALIDATION; NOT_PROMOTED_WITHOUT_FINAL_REVIEW",
  };
  if (process.env.STUDENTHUB_PERSIST_LIVE_HOLDOUT === "1") {
    fs.writeFileSync(
      path.resolve("artifacts/retrieval/fresh_retrieval_holdout_v4_results.json"),
  ...
  // This test enforces safety invariants. External-recall targets are reported
  // honestly in the artifact and cannot be turned green by gold injection.
  assert.equal(poolIntegrityPass, true, "RC3 hybrid pool must preserve static, live and official discovery candidates");
  assert.equal(recallDelta >= -1, true, `RC3 Hybrid Recall@5 must not regress more than 1pp vs LIVE, got ${recallDelta.toFixed(1)}pp`);
});
