...
test.describe("Community evidence layer", () => {
  test("search, topic filters and provenance remain explicit", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: /Trải nghiệm thật/ })).toBeVisible();
    await expect(page.getByText("LIVE PROVIDER").first()).toBeVisible();
    await expect(page.getByPlaceholder("Tìm vấn đề, quy trình hoặc bằng chứng...")).toBeVisible();
    ...
    await expect(page.getByText(/Không có báo cáo phù hợp với bộ lọc hiện tại\.|Nguồn chưa khả dụng/).first()).toBeVisible();
  });
