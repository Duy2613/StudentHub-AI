...
- `STUDENTHUB_STAGING_CASES_PATH`: absolute path to an operator-owned JSON file with `suspicious`, `benign`, `partial`, `insufficient`, and `failure` cases.
...
Each case contains `type` (`text` or `url`) and `value`. Optional assertions are `expected`, `expectedProviderStatuses`, and `expectedErrorFragment`. The staging operator/backend test controls must make the failure case produce a real 429/502/503/timeout; the frontend suite does not fake it.
