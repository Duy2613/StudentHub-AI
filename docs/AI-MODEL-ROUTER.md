...
This AI Gateway replaces that with a real, working, capability-based
**ModelRouter** that rotates across every currently-available provider with
automatic fallback, timeout handling, and structured-output validation —
while keeping every deterministic engine as the default and authoritative
path (zero regression risk; see §5).
...
Because both integrations are strictly opt-in via `options.useAIGateway`,
**every one of the existing 700+ deterministic tests keeps passing
unmodified** — confirmed by running the full suite (`npm run test:all` at
both `frontend/` and repo root) after this change: 417/417 pass, 0 failures.
...
Live end-to-end verification against the real GenSpark LLM proxy was
performed manually during development (see PR description) and is not part
of the automated suite, to avoid spending credits / flaking CI on network
conditions.
